...
class Agent:
    def __init__(
        self,
        task: Union[str, List[str]],  # <--- Accept single task or list of tasks
        llm: BaseChatModel,
        ...
        self.task = self.tasks[0] if self.tasks else "No Task Provided"
    ...
    def _create_message_manager(self, task: str) -> MessageManager:
        """
        Create and configure a new MessageManager instance for a given task.
        """
    ...
    async def run(self, max_steps: int = 100) -> AgentHistoryList:
        ...
        try:
            for idx, task in enumerate(self.tasks, start=1):
                ...
                self.message_manager = self._create_message_manager(self.task)
                self._log_agent_run()
                ...
                for _ in range(max_steps):
                    ...
                    logger.info(
                        f"❌ Failed to complete sub-task in maximum steps: {task}"
                    )
    ...
    def _create_task_frame(
        ...
        bigger_font = (
            ImageFont.truetype(regular_font.path, regular_font.size + 16)
